...
export default function HistoryLog({ logs, onDeleteLog }: HistoryLogProps) {
  ...
  // Category Utilities
  const categoryConfig: Record<'transport' | 'energy' | 'food' | 'shopping', { icon: React.ReactNode; color: string; bg: string }> = {
  ...
  const chart = useMemo(() => {
    if (logs.length === 0) return { path: '', area: '', points: [] as ChartPoint[], dates: [] as string[] };
    ...
    const sortedDates = Object.keys(grouped).sort((a, b) => new Date(a).getTime() - new Date(b).getTime()).slice(-7);
    const values = sortedDates.map(date => grouped[date]);
    
    if (sortedDates.length === 0) return { path: '', area: '', points: [] as ChartPoint[], dates: [] as string[] };
    ...
    const points: ChartPoint[] = sortedDates.map((date, idx) => {
