...
type CategoryType = 'transport' | 'energy' | 'food' | 'shopping' | 'none';
...
export default function Recommendations({ 
  ...
  const categoriesSum = useMemo(() => {
    return logs.reduce((acc, log) => {
      ...
      return acc;
    }, { transport: 0, energy: 0, food: 0, shopping: 0 } as Record<'transport' | 'energy' | 'food' | 'shopping', number>);
  }, [logs]);
  ...
  const highestStats = useMemo(() => {
    let highestCategory: CategoryType = 'none';
    let highestValue = 0;
    
    (Object.keys(categoriesSum) as Array<'transport' | 'energy' | 'food' | 'shopping'>).forEach(cat => {
  ...
  const catDetails: Record<CategoryType, CategoryDetail> = {
